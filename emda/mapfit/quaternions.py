...
def get_quaternion(theta):
    ...
    q1 = rv[0]*np.sin(theta/2.0)
    q2 = rv[1]*np.sin(theta/2.0)
    q3 = rv[2]*np.sin(theta/2.0)
...
def q_normalised(rv):
    ...
    q2 = np.array([q02, abs(rv[1]), abs(rv[2]), abs(rv[3])])
    q = np.sqrt(q2)
    return q
