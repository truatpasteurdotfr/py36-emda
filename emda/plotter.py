...
def plot_nlines(res_arr,
    ...
    #
    #data = np.loadtxt("/Users/ranganaw/MRC/Map_superposition/mapfit01/codes/classes/code4_20181102/table2plot.txt", skiprows=1)
    #fig = plt.figure()
    #ax1 = fig.add_subplot(111)

    # You have to use integer indices for the x axis.
    # Otherwise get_xticks() will return non-integers.
    #ax1.plot(data[:,0], data[:,2])

    pos = np.array(ax1.get_xticks(), dtype=np.int)
    # This contains ticks outside the visible range,
    # so it must be clipped
    #print(pos)
    ...
    #ax1.set_xticklabels(np.round(data[pos, 1], decimals=2))
    ax1.set_xticklabels(np.round(res_arr[pos], decimals=2))
    ax1.set_xlabel("Resolution ($\AA$)")
    #plt.show()
    #
