...
def randomize_phases(sf):
    ...
    sf_phase_randomized = amplitudes * np.cos(random_phases) + 1j * amplitudes * np.sin(random_phases)
    return sf_phase_randomized

def get_randomized_sf(uc,rho,resol_randomize=10.0):
    print('SF phase randomization')
    f_ori = np.fft.fftshift(np.fft.fftn(rho))
    f_all_random = randomize_phases(f_ori)
    nx,ny,nz = f_ori.shape
    f_beyond_random = fcodes.add_random_phase_beyond(f_ori,f_all_random,uc,resol_randomize,nx,ny,nz)
